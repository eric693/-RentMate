...
export const DOCUMENT_VARIABLES: { key: string; label: string }[] = [
...
export const DEFAULT_TEMPLATE = `住宅租賃契約書
...
async function buildContext(contractId: string): Promise<RenderContext> {
  ...
  const months = Math.max(
    1,
    Math.round((c.endDate.getTime() - c.startDate.getTime()) / (30.44 * 86400000)),
  );
...
export async function getDocument(contractId: string) {
  ...
  const body = contract.documentBody ?? DEFAULT_TEMPLATE;
  return {
    body,
    ...
    variables: DOCUMENT_VARIABLES.map((v) => ({ ...v, value: ctx[v.key] ?? '' })),
  };
}

/** 取得代入後的完稿（給租客端簽署頁與 LINE 用）。 */
export async function renderDocument(contractId: string): Promise<string> {
  const contract = await prisma.contract.findUniqueOrThrow({ where: { id: contractId } });
  const ctx = await buildContext(contractId);
  return renderTemplate(contract.documentBody ?? DEFAULT_TEMPLATE, ctx);
}
