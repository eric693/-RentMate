...
export async function getCollectionWorkbench(req: AuthRequest, res: Response) {
  ...
  for (const r of rentRecords) {
    tasks.push({
      ...
      status: r.status.toLowerCase() as TaskStatus,
  ...
  for (const e of expenses) {
    const labelMap: Record<string, string> = { WATER: '水費', ELECTRICITY: '電費', GAS: '瓦斯費' };
...
export async function getFinanceOverview(req: AuthRequest, res: Response) {
  ...
  async function getMonthStats(y: number, m: number) {
    const records = await prisma.rentRecord.findMany({ where: { contractId: { in: contractIds }, year: y, month: m } });
  ...
  for (const e of expenseRaw) { byCategory[e.category] = (byCategory[e.category] ?? 0) + Number(e.amount); }
