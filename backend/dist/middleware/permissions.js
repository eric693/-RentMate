...
exports.MODULE_KEYS = exports.MODULES = void 0;
exports.moduleForPath = moduleForPath;
...
exports.MODULE_KEYS = exports.MODULES.map((m) => m.key);
/**
...
function moduleForPath(path) {
    for (const [re, key] of PATH_MODULES)
        if (re.test(path))
            return key;
    return null;
}
function sanitizePermissions(input) {
    if (!Array.isArray(input))
        return [];
    return [...new Set(input.filter((k) => typeof k === 'string' && exports.MODULE_KEYS.includes(k)))];
}
