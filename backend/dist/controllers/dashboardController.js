...
async function getDashboard(req, res) {
    ...
    const allUnits = await app_1.prisma.unit.findMany({ where: { propertyId: { in: propertyIds } } });
    ...
    const collectedRent = rentRecords
        .filter((r) => r.status === client_1.RentStatus.PAID || r.status === client_1.RentStatus.PARTIAL)
        .reduce((sum, r) => sum + Number(r.paidAmount ?? r.amount), 0);
    const paidCount = rentRecords.filter((r) => r.status === client_1.RentStatus.PAID).length;
    const pendingCount = rentRecords.filter((r) => r.status === client_1.RentStatus.PENDING).length;
    const overdueCount = rentRecords.filter((r) => r.status === client_1.RentStatus.OVERDUE).length;
    const overdueAmount = rentRecords
        .filter((r) => r.status === client_1.RentStatus.OVERDUE)
        .reduce((sum, r) => sum + Number(r.amount), 0);
    const pendingMaintenance = await app_1.prisma.maintenanceRequest.count({
        where: { unitId: { in: unitIds }, status: { in: [client_1.MaintenanceStatus.PENDING, client_1.MaintenanceStatus.IN_PROGRESS] } },
    });
    const trendData = [];
    for (let i = 5; i >= 0; i--) {
    ...
    const summaryParts = [];
    if (overdueCount > 0)
        summaryParts.push(`${overdueCount} 筆逾期租金`);
    if (expiringContracts.length > 0)
        summaryParts.push(`${expiringContracts.length} 份即將到期合約`);
    if (pendingMaintenance > 0)
        summaryParts.push(`${pendingMaintenance} 筆待追蹤報修`);
